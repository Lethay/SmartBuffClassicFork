...
function SMARTBUFF_SyncBuffTimer(unit, grp, cBuff, isGrpBuff)
  ...
  if (d and d > 0 and buff) then
    local t = GetTime();
    local ret, _, _, timeleft = SMARTBUFF_CheckUnitBuffs(unit, buff, cBuff.Type, cBuff.Links, cBuff.Chain, cBuff.BuffG);
    ...
    if (cGrp[subgroup] ~= nil or (type(subgroup) == "number" and subgroup == 1)) then
      ...
      if (buffs and unitsGrp and not isMounted) then
        ...
        while (cBuffs[i] and cBuffs[i].BuffS) do
          ...
          -- print( --FIXME: Delete this
          --   "i:", i,
          --   "buffnG:", buffnG,
          --   "tmpDisabled[buffnG] == nil:", tmpDisabled[buffnG] == nil,
          --   "bs:", bs,
          --   "bs.EnableG:", bs.EnableG,
          --   "cBuff.IDG ~= nil:", cBuff.IDG ~= nil,
          --   "((isCombat and bs.CIn) or (not isCombat and bs.COut)):", ((isCombat and bs.CIn) or (not isCombat and bs.COut)),
          --   "IsPowerLimitOk(bs):", IsPowerLimitOk(bs),
          --   "if statement:",  (buffnG and tmpDisabled[buffnG] == nil and bs and bs.EnableG and cBuff.IDG ~= nil
          --   and ((isCombat and bs.CIn) or (not isCombat and bs.COut)) and IsPowerLimitOk(bs)
          --   --and UnitMana("player") >= bs.ManaLimit
          --   and (sPlayerClass ~= "PALADIN" or not cClassGroups or (sPlayerClass == "PALADIN" and (bs[subgroup] or (type(subgroup) == "number" and subgroup == 0)))) )
          -- )
...
function SMARTBUFF_BuffUnit(unit, subgroup, mode, spell)
  ...
  if (UnitExists(unit) and UnitIsFriend("player", unit) and not UnitIsDeadOrGhost(unit) and not UnitIsCorpse(unit)
    ...
    for i, buffnS in pairs(B[CS()].Order) do    
      ...
      --buffnS = cBuff.BuffS; --already set in loop!
      buffnG = cBuff.BuffG
          ...
          if (cd <= 0 or (mode == 1 and cd <= 1.5)) then
            ...
            if (not SMARTBUFF_IsInList(unit, un, bs.IgnoreList) and (((cBuff.Type == SMARTBUFF_CONST_GROUP or cBuff.Type == SMARTBUFF_CONST_ITEMGROUP)
              ...
              elseif (cBuff.Type == SMARTBUFF_CONST_FOOD or cBuff.Type == SMARTBUFF_CONST_SCROLL or cBuff.Type == SMARTBUFF_CONST_POTION or 
                ...
                elseif (cBuff.Type == SMARTBUFF_CONST_FOOD) then
                  if (not SMARTBUFF_IsPicnic(unit)) then
                    buff, index, buffname, bt, charges = SMARTBUFF_CheckUnitBuffs(unit, SMARTBUFF_FOOD_AURA, cBuff.Type, cBuff.Links, cBuff.Chain, nil);
                  end
                else
                  if (cBuff.Params ~= SG.NIL) then
                    if (cBuff.Links and cBuff.Links == SG.CheckFishingPole) then
                      if (SMARTBUFF_IsFishingPoleEquiped()) then
                        buff, index, buffname, bt, charges = SMARTBUFF_CheckUnitBuffs(unit, cBuff.Params, cBuff.Type, nil);
                      else
                        buff = nil;
                      end
                    else
                      buff, index, buffname, bt, charges = SMARTBUFF_CheckUnitBuffs(unit, cBuff.Params, cBuff.Type, cBuff.Links, cBuff.Chain, nil);
                    end
              ...
              -- Normal buff ------------------------------------------------------------------------
                ...
                -- check timer object
                buff, index, buffname, bt, charges = SMARTBUFF_CheckUnitBuffs(unit, buffnS, cBuff.Type, cBuff.Links, cBuff.Chain, buffnG); --TODO: Change to fix group buffs: now has buffnG at the end
                if (charges == nil) then charges = -1; end
                if (charges > 1) then cBuff.CanCharge = true; end
...
function SMARTBUFF_CheckUnitBuffs(unit, buffN, buffT, buffL, buffC, buffG) --TODO: Change for group buffs: now has buffG at the end
  ...
  -- Check group buff --TODO: This is a new section for group buffs
  if (buffG) then
    buff, icon, count, _, duration, timeleft, caster = UnitBuffByBuffName(unit, buffG);
    if (buff) then
      timeleft = timeleft - time;
      SMARTBUFF_AddMsgD("Linked buff found: "..buff..", "..timeleft..", "..icon);
      return nil, n, defBuff, timeleft, count;
...
function SMARTBUFF_IsPicnic(unit)
  if (not SMARTBUFF_CheckUnitBuffs(unit, SMARTBUFF_FOOD_SPELL, SMARTBUFF_CONST_FOOD, {SMARTBUFF_FOOD_SPELL, SMARTBUFF_DRINK_SPELL}, nil)) then
    return true;
  end
  return false;
end
...
function SMARTBUFF_IsActiveBattlefield(zone)
  ...
  for i = 1, GetMaxBattlefieldID() do
    ...
    if (status and status ~= "none") then
      -- SMARTBUFF_AddMsgD("Battlefield status = "..ChkS(status)..", Id = "..ChkS(instanceId)..", TS = "..ChkS(teamSize)..", Map = "..ChkS(map)..", Zone = "..ChkS(zone)); --FIXME: This doesn't seem to work!
...
function SMARTBUFF_OToggleBuff(s, i)
  ...
  if (s == "S") then
    ...
    cBuff = cBuffs[i] --TODO: This and the line below are the potential fix to smartbuff; post to curse
    if (cBuff.IDG ~= nil) then
...
local function OnScroll(self, cData, sBtnName)
  ...
  for i = 1, maxScrollButtons, 1 do
    ...
    --TODO: What about group buff buttons?
  end
end
